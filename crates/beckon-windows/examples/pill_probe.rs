...
// **`fn main` first, and the Windows half inside `mod win`** -- the shape
// every other probe in this directory uses, and this file did not until
// 2026-08-16. It carried `#![cfg(target_os = "windows")]`, a CRATE-level inner
// attribute, which on a non-Windows host removes the entire file including
// `main`, so the example failed to compile with E0601 rather than compiling to
// a no-op. Nothing caught it while the workspace build excluded
// `beckon-windows` on the two non-Windows CI jobs; the `unexcluded` step added
// with the nix fix is what surfaced it.
fn main() {
    #[cfg(not(target_os = "windows"))]
    eprintln!("pill_probe only does anything on Windows");
