...
| The editor strip is ONE control line | **changed, and it had never been written down — found by audit 2026-08-16.** The mock-up puts the whole editor on one `.srow`: `App` at 168 px, the four chips, the key list at 74 px, then `Record` and `Revert` right-aligned; the note is the only thing under it. Measured in Chrome, that card is 92.9 px and the row inside it is 46. The window draws **two** control lines — `App` alone across the full card width, then chips / key / `Record` / `Revert` — so `card2_h` spends `ctl + gap` (32 px at 96 DPI) the drawing does not. **Keep it.** The reason is the design's own: §11 rejects the Bench layout partly because it takes the App column 485 → 260 px and "long PWA names truncate", and a Brave PWA's display name is the longest string this window ever shows. One line would put `App` at roughly 120-170 px against `tok::BTN` 88 twice, four chips and the key list. What was wrong was only that nothing said so — `layout.rs` describes the two lines and never mentions the drawing has one, which is exactly how a later pass "fixes" it back |
...
| The Build row reads `<target> · <date>` | **open — found by audit 2026-08-16, and it is an omission rather than a deviation.** §3.4's drawing is `Build   aarch64-pc-windows-msvc · 2026-08-13`; the window draws the triple alone. It is not a matter of an unset field: `AboutInputs` has **no date member at all**, so nothing anywhere carries a build timestamp into this page — `about_state` sets `build.shown` and `build.copy` both to `i.target`. Cheap to close and worth weighing against what landed since: `beckon-cli` gained a `build.rs` that puts a short SHA in `--version`, which answers "which build" more precisely than a date does. `beckon-windows` deliberately did NOT follow (it still uses its own `env!("CARGO_PKG_VERSION")`), so closing this means either a `build.rs` for that crate or a date, and the design asked for the date. **Note what does NOT depend on it:** the a14 incident this page exists for is answered by `Location` plus `image_age`, both built and both working — this row is the identifying detail, not the safety net |
