...
**The App combo box.** A populated `CBS_DROPDOWN` rewrites its own text as you type, and the `CBN_EDITCHANGE` that arrives carries the text from *before* the rewrite — measured on a14: typing "Notepad" wrote `"d"` to the config while the screen showed something else. `commit_fields` papers over it at Apply time; the field itself still lies while you type. This is the last remaining instance of a defect the rest of the design engineered out, and the spec makes it a gate for this landing.
