...
| **eye** | Open the rebuilt window at 96 DPI **and** 150 %: group caption tracks the selection; empty state on deselect; two lines never overlap at `MIN_WIDTH` 720; `Ctrl + Win + Alt + T` in the Shortcut column; notes never exceed two lines. | — | The two-line cap and the `MIN_WIDTH` floor are the two things unit tests structurally cannot see. |
